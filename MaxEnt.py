...
class MaxEnt:
    def __init__(self, epsilon=1e-3, maxstep=100):
        ...
        self.maxstep = maxstep  # 最大训练步骤
        self.w = None  # 特征函数的权重
        self.labels = None  # 标签
        self.fea_list = []  # 特征函数
        self.px = defaultdict(lambda: 0)  # 经验边缘分布概率
        """ 经验联合分布概率,由于特征函数为取值为0，1的二值函数，所以等同于特征的经验期望值"""
    ...
    def predict(self, X):
        ...
        # 取出分类和对应的值
        # print("key == " + str(best_label) + " value == " + str(py_x[best_label]))
        # return best_label
        return {'label': best_label, 'score' : py_x[best_label]}